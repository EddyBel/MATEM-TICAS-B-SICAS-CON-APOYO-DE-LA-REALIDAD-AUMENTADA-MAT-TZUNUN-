...
class ModeloIA(torch.nn.Module):
    def __init__(self):
        ...
        self.layer1 = torch.nn.Linear(4, 16)  # Capa de entrada con 4 características y 16 neuronas
        self.layer2 = torch.nn.Linear(16, 8)   # Capa oculta con 8 unidades
        self.output_layer = torch.nn.Linear(8, 3)  # Capa de salida con 3 clases
...
class Prediccion:
    def __init__(self):
        ...
        try:
            state_dict = torch.load('SRC/Core/modelo_decision_niveles.pth', weights_only=True)
        ...
        X_train = np.array([[12, 13.2, 55, 2], [11, 14.0, 60, 3], [10, 12.0, 50, 1]])  # Datos de entrenamiento reales
        self.escalador.fit(X_train)  # Ajustar el escalador con los datos de entrenamiento

    def predecir(self, tupla):
        # Normalizar los datos de entrada
        data_input = np.array(tupla).reshape(1, -1)  # Convertir a array y dar forma (1, 4)
        data_input_escalada = self.escalador.transform(data_input)  # Escalar los datos

        # Convertir a tensor
        data_input_tensor = torch.tensor(data_input_escalada, dtype=torch.float32)
